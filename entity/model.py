...
class LinearLayer(nn.Module):
    ...
    def __init__(self):
        ...
        self.cosd = nn.Linear(self.dimension,self.dimension)
        self.eucd  = nn.Linear(self.dimension,self.dimension)
        self.paird = nn.Linear(self.dimension,self.dimension)

        self.time = nn.Linear(self.dimension,self.dimension)
        self.cosdf = nn.Linear(self.dimension,self.dimension)
        self.eucdf  = nn.Linear(self.dimension,self.dimension)
        self.pairdf = nn.Linear(self.dimension,self.dimension)
        ...
        self.concat = nn.Linear(self.dimension*4,self.dimension)
        self.concatf = nn.Linear(self.dimension*4,self.dimension)

        self.concat2 = nn.Linear(2*self.dimension,int(self.dimension/3))
        self.classifier = nn.Linear(int(self.dimension/3),1)
    ...
    def forward(self, pairwise, cos, euclid, time_diff, pairwisef, cosf, euclidf):
        ...
        ### d9
        time_diff =  time_diff.repeat(1,self.dimension)
        time_diff = self.time(time_diff)

        pairwise =  pairwise.repeat(1,self.dimension)
        pairwise = self.dropout(self.relu(self.paird(pairwise)))

        cos =  cos.repeat(1,self.dimension)
        cos = self.dropout(self.relu(self.cosd(cos)))

        euclid =  euclid.repeat(1,self.dimension)
        euclid = self.dropout(self.relu(self.eucd(euclid)))


        pairwisef =  pairwisef.repeat(1,self.dimension)
        pairwisef = self.dropout(self.relu(self.pairdf(pairwisef)))

        cosf =  cosf.repeat(1,self.dimension)
        cosf = self.dropout(self.relu(self.cosdf(cosf)))

        euclidf =  euclidf.repeat(1,self.dimension)
