...
def main():
            ...
            for js in joint_ann_score:
                ...
                for rs in relatedness_score:
                    ...
                    for bs in batch_size:
                        ...
                        v_samples,v_label,v_session  = helper.reshaped_combine_seperate(args,df2, w2v,256, Query_emb_dict1, True)
    ...
    ################################################ TEST #####################
        ...
        model.load_state_dict(copy.deepcopy(torch.load("siamese_best_for_test.pth", torch.device("cuda:0"))))
        model.eval()
        tr= Train(model, 128, 2e-3, args, w2v, Query_emb_dict1)
    ...
    ############################################# task extraction ########################
        ...
        all_userid, all_label, t_samples, label,  session= helper.get_test_data(args.test_data, Query_emb_dict1)

        #t_v1,t_v2,label,session, samples = WIKI.data_loader (w2v,[1], list(zip(query_list, query_list)), labels, session )

        #label = sum(label, [])
        helper.end_to_end(all_userid, all_label, tr, t_samples, label,  session, w2v_fastext, w2v1,start_time,time)
